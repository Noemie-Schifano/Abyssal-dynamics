...
file_buoyb      = '/home/datawork-lops-rrex/nschifan/DIAGS/rrexnumsb200-rsup5_16tracers_6h_k_buoy_rhop_tracer_balance.nc'
...
#buoy_dv  = nc.variables['buoy_avg_dv'][:,:].T    # buoy(ntpas,nt) 
...
'''
# ----------- read k and b with each rho_p per tracer ---------------
if test_sigma1_k ==True:
    nc1 = Dataset(file_k1_s1,'r')
    buoy1     = nc1.variables['buoy_avg'][:,:].T    # buoy(ntpas,nt) 
    Ktr1      = nc1.variables['Ktr'][:].T
    nc1.close()

    nc2 = Dataset(file_k2_s1,'r')
    buoy2     = nc2.variables['buoy_avg'][:,:].T    # buoy(ntpas,nt) 
    Ktr2      = nc2.variables['Ktr'][:].T
    nc2.close()


else:
    nc1 = Dataset(file_k1,'r')
    buoy1     = nc1.variables['buoy_avg'][:,:].T    # buoy(ntpas,nt) 
    Ktr1      = nc1.variables['Ktr'][:].T
    nc1.close()

    nc2 = Dataset(file_k2,'r')
    buoy2     = nc2.variables['buoy_avg'][:,:].T    # buoy(ntpas,nt) 
    Ktr2      = nc2.variables['Ktr'][:].T
    nc2.close()

Ktr_corrected  = np.zeros((nt,ntpas))
buoy_corrected = np.zeros((nt,ntpas))
Ktr_corrected[:,0:8]   = Ktr1
Ktr_corrected[:,8:]  = Ktr2
buoy_corrected[:,0:8]  = buoy1
buoy_corrected[:,8:] = buoy2
'''

# --- Make plot ---
if plot_buoy_balance==True:
	nc2         = Dataset('/home/datawork-lops-rrex/nschifan/DIAGS/rrexnumsb200-rsup5_16T_6h_porcentage_in_BBL.nc','r')
	...
	plt.savefig('/home/datawork-lops-rrex/nschifan/Figures/WMT/Deep_tracer_6h/diags_tracer_w_N2_buoy_balance.png',dpi=200,bbox_inches='tight')
...
if plot_tracer_b == True:
    ...
    plt.savefig('/home/datawork-lops-rrex/nschifan/Figures/WMT/Deep_tracer_6h/16tracer_b.png',dpi=180,bbox_inches='tight')
    plt.close()
